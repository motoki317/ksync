...
// caller resolves the context (the sole allowedContexts entry, or --context) and
// validates it against ksync.yaml's allowedContexts before reaching this point,
// so an empty name is a programming error, not a fall-through to whatever happens
// to be current.
